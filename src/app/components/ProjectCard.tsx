...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, isExpanded, onClick }) => {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className={`w-full ${isExpanded ? "h-auto" : "h-48"} overflow-hidden transition-all duration-500 ease-in-out`}>
        <Swiper
            ...
            <SwiperSlide key={index}>
              <div className="relative w-full h-40 md:h-48 flex justify-center items-center bg-gray-100">
                <Image
                  ...
                  fill
                  className="object-contain w-full h-auto transition-all duration-500 ease-in-out"
                  sizes="100vw"
                />
              </div>
            </SwiperSlide>
